...
const GRADIENTS = [
  ...
  "linear-gradient(45deg, #5eeff9 0%, #4facfe 100%)",
...
export function NoteCard({ note, onClick, className }: NoteCardProps) {
  ...
  const dateStr = note.createdAt ? format(note.createdAt.toDate(), "dd MMM", { locale: fr }) : "..."
  
  const gradientIndex = React.useMemo(() => {
    if (!note.id) return 0;
    const charSum = note.id.split('').reduce((acc: number, char: string) => acc + char.charCodeAt(0), 0);
    return charSum % GRADIENTS.length;
  }, [note.id]);
  ...
  return (
    <div 
      className={cn("universe-card relative group", className)}
      onClick={onClick}
    >
      <button 
        onClick={toggleFavorite}
        className={cn(
          "absolute top-2 right-2 z-30 p-1.5 rounded-full transition-all hover:scale-110",
          note.isFavorite ? "text-yellow-400" : "text-white/40 hover:text-white"
        )}
      >
        <Star className={cn("h-5 w-5", note.isFavorite && "fill-current")} />
      </button>

      <div className="top-section" style={{ background: randomGradient }}>
        ...
        <div className="flex-1 flex items-center justify-center p-6 text-center mt-6">
          <p className="text-white text-[10px] font-medium leading-relaxed line-clamp-3 opacity-90 italic">
            "{note.description?.substring(0, 100).replace(/[#*`]/g, '') || "Contenu technique en attente..."}"
          </p>
        </div>
      </div>

      <div className="bottom-section">
        <span className="title truncate px-2">{note.title}</span>
        <div className="row">
          <div className="item">
            <span className="big-text">{dateStr}</span>
            <span className="regular-text">MODIFIÉ</span>
          </div>
          <div className="item">
            <span className="big-text uppercase truncate px-1">{note.template || "Standard"}</span>
            <span className="regular-text">MODÈLE</span>
          </div>
          <div className="item">
            <span className="big-text truncate px-1 uppercase">
              {note.tags && note.tags.length > 0 ? note.tags[0] : "TECH"}
            </span>
            <span className="regular-text">TAG</span>
          </div>
        </div>
      </div>
    </div>
